...
//		a = a + 1; // 컴파일에러 발생(byte + int = int 이므로 저장 불가)
		...
		// => 변수 x1 의 값을 먼저 1만큼 증가시킨 후
		//    대입연산자를 통해 x1 값을 y1 에 대입(저장)
		// => 따라서, x1 값이 5 -> 6 으로 증가된 후 y1에 증가된 값 6이 저장됨
		System.out.println(x1 + ", " + y1); // 6, 6
		
		int x2 = 5;
		int y2 = x2++; // 후위연산자(후행연산자)
		// => 변수 x2 의 값을 먼저 y2에 대입한 후
		//    증감연산자를 통해 x2 값을 1만큼 증가시킴
		// => 따라서, x2 값이 y2에 저장된 후, x2 값이 5 -> 6 으로 증가
		System.out.println(x2 + ", " + y2); // 6, 5
		
		System.out.println("--------------------------");
		
		// 대입연산자와 결합 대신 출력문에서 사용 가능
		x2 = 5;
		System.out.println(++x2); // x2 값을 먼저 1 증가시킨 후 출력(6출력)
		// => 출력문에 x2 값을 전달하기 전에 먼저 증감연산자가 사용됨
		
		System.out.println(x2++); // x2 값을 먼저 출력한 후 1 증가시킴(6출력)
		// => 출력문에 x2 값을 먼저 전달한 후 증감연산자가 사용됨
		
		// 위의 과정에서 6이 출력된 후 6 -> 7 로 증가되었으므로 7이 출력
		...
		// ch 값을 직접 변경하지 않았으므로
		System.out.println(ch); // 변경 전 값임 'A' 출력됨
